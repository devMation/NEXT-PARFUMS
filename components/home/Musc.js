...
// mon fichier qui contient le tableau d'objet contenu dans MuscData
const Musc = ({ MuscData }) => {
  return (
    // je englobe dans une div tous le processus de bouclage qui permettre d'afficher mais articles
    // je cree une boucle map() qui va permettre de parcourir le tableau musc
    <div id="musc">
      Musc
      {MuscData.map((item, index) => (
...
export default Musc; // je vais exporter ma fonction qui se nomme (Musc).
